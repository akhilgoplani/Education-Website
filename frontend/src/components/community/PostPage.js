...
class PostPage extends Component {
  ...
  checkLike() {
    ...
    console.log(user);
    if (user && post.upvotes && post.upvotes.includes(user.id)) return true;
    else return false;
  }
  checkReplyLike(id) {
    ...
    if (user) {
      for (let i in replies) {
        if (replies[i]._id === id) {
          if (replies[i].upvotes.includes(user.id)) return true;
        }
      }
    }
    return false;
      ...
      <div>
        ...
        <div className="container col-lg-6 shadow-lg p-3 mt-5 bg-body rounded">
          ...
          <div className="mt-1">
            ...
            {post.tags &&
              post.tags.map((tag) => (
                <span className=" text-success  m-1 p-2">{tag.name}</span>
              ))}
